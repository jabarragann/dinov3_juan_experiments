from dinov3_lib_juan.datasets import tudl_bop
...
def benchmark_launcher(eval_args: dict[str, object]) -> dict[str, Any]:
    ...
    if "config" in eval_args:  # using a config yaml file, useful for training
        ...
        dataclass_config, output_dir = args_dict_to_dataclass(
            eval_args=eval_args, config_dataclass=SegmentationConfig
        )
    ...
    # else:
    #     assert dataclass_config.load_from == "dinov3_vit7b16_ms", f"currently {dataclass_config.load_from}"
